...
namespace Gerador_de_Folha_de_Pagamento_Desktop.Apresentacao
{
    public partial class frm_Cadastrar_Folhas_Pagamento : Form
    {
        ...
        {
            // Verifica se a tecla pressionada é um número, uma vírgula ou outra tecla
            bool numero = Char.IsNumber(e.KeyChar);
            bool outra_tecla = Char.IsControl(e.KeyChar);
            bool virgula = e.KeyChar == ',';

            // Se a tecla pressionada for uma vírgula, verifica se já existe uma no texto
            if (virgula)
            {
                if (txb_Valor_Hora.Text.Contains(","))
                {
                    // Se já houver uma vírgula, ignora a tecla pressionada
                    e.Handled = true;
                }
            }
            ...
            {
                // Se a tecla pressionada não for um número nem um controle, ignora a tecla pressionada
                e.Handled = true;
            }
        }
        ...
        {
            if (cmb_Selecionar_Funcionario.SelectedIndex == -1 || txb_CPF.Text == "")
            {
                MessageBox.Show("Selecione um Funcionário para " +
                    "cadastrar uma Folha de Pagamento", "Aviso", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }
            ...
            else
            {
                ...
                if (Controle_Validacao.Folha_Pagamento_Validado == true)
                {
                    ...
                    controle_validacao2.Verificar_Cadastro_Folha_Pagamento(folha_pagamento, CPF_Funcionario_Selecionado);

                    if (Controle_Validacao.Cadastro_Folha_De_Pagamento_Validado == true && Folha_Pagamento_DAO.Cadastro_Folha_De_Pagamento_Realizado == true)
                    {
                        Controle_Validacao.Cadastro_Folha_De_Pagamento_Validado = false;
                        Folha_Pagamento_DAO.Cadastro_Folha_De_Pagamento_Realizado = false;
